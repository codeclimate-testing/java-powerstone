...
public class FlowModelDAO {
  ...
  private HashMap TypeDeclarations;
...
//------------------------------------------------------------------------------
  public HashMap getTypeDeclarations() {
    ...
    Element declarations[] =
        ElementUtils.findElements(root, "TypeDeclaration", null, null, null);
    if (declarations != null && declarations.length > 0) {
      for (int i = 0; i < declarations.length; i++) {
        Element temTD = declarations[i];
        String declareID = temTD.getAttributeValue("Id");
        Element enumerations[] =
            ElementUtils.findElements(temTD,
                                      "EnumerationValue",
                                      null, null, null);
...
//------------------------------------------------------------------------------
  public String[] getFlowVariablesToPreview(String flowProcessID) {
    ...
    String val[] = new String[] {
        FlowMetaConstants.FLOW_PREVIEW_VARIABLE};
    Element[] elemArray = ElementUtils.findElements(processRoot, null, null,
        attr, val);
  ...
  private Element getFlowProcessRoot(String flowProcessID) {
    try {
      ...
      if (processRoot == null || processRoot.length < 1) {
        throw new RuntimeException("找不到根结点：" + flowProcessID);
      }
      else {
...
//------------------------------------------------------------------------------
  public String getStartNodeID(String flowProcessID) {
    ...
    Element[] elemArray =
        ElementUtils.findElements(processRoot, null, null, attr, val);
    if (elemArray == null || elemArray.length == 0) {
      throw new RuntimeException("找不到开始结点：" + flowProcessID);
    }
    else {
...
//------------------------------------------------------------------------------
  public String[] getEndNodeIDs(String flowProcessID) {
    ...
    Element[] elemArray = ElementUtils.findElements(processRoot, null, null,
        attr, val);
    if (elemArray == null || elemArray.length == 0) {
      throw new RuntimeException("找不到结束结点：" + flowProcessID);
    }
    else {
...
//------------------------------------------------------------------------------
  public WorkflowNode findWorkflowNodeByID(String flowProcessID,
                                           String nodeID) {
    Element processRoot = this.getFlowProcessRoot(flowProcessID);
    String attr[] = new String[] {
        "Id"};
    ...
    Element[] elemArray =
        ElementUtils.findElements(processRoot, "Activity", null,
                                  attr, val);
    if (elemArray == null || elemArray.length == 0) {
      throw new RuntimeException("不存在的工作流节点:" + nodeID + "!");
    }
    else {
      String activityName = "";
      String activityType = "";
      ...
      if (temImp != null && temImp.length > 0) {
        Element typeNo = temImp[0].getChild(FlowMetaConstants.ACTIVITY_TYPE_NO);
        Element typeSubFlow = temImp[0].getChild(FlowMetaConstants.
                                                 ACTIVITY_TYPE_SUBFLOW);
        Element typeTool = temImp[0].getChild(FlowMetaConstants.
                                              ACTIVITY_TYPE_TOOL);
        if (typeNo != null) {
      ...
      attr = new String[] {
          "Name"};
      val = new String[] {
          FlowMetaConstants.STATE_ID};
      Element[] temStateID = ElementUtils.findElements(activityRoot, null, null,
          attr, val);
      ...
      attr = new String[] {
          "Name"};
      val = new String[] {
          FlowMetaConstants.STATE_NAME};
      Element[] temStateName = ElementUtils.findElements(activityRoot, null, null,
          attr, val);
      ...
      attr = new String[] {
          "Name"};
      val = new String[] {
          FlowMetaConstants.PERSON_STATE};
      Element[] temPersonState = ElementUtils.findElements(activityRoot, null, null,
          attr, val);
      ...
      attr = new String[] {
          "Name"};
      val = new String[] {
          FlowMetaConstants.DISTRIBUTABLE};
      Element[] temDistributable =
          ElementUtils.findElements(activityRoot, null, null, attr, val);
      ...
      attr = new String[] {
          "Name"};
      val = new String[] {
          FlowMetaConstants.VARIABLE_TO_PROCESS_OUT};
      Element[] temVariableOut = ElementUtils.findElements(activityRoot, null, null,
          attr, val);
      ...
      if (temVariableOut != null && temVariableOut.length > 0) {
        ...
        for (int i = 0; i < temVariableOut.length; i++) {
          variablesOut[i] = temVariableOut[i].getAttributeValue("Value");
        }
      }
      ...
      attr = new String[] {
          "Name"};
      val = new String[] {
          FlowMetaConstants.VARIABLE_TO_PROCESS_IN};
      Element[] temVariableIn = ElementUtils.findElements(activityRoot, null, null,
          attr, val);
      if (temVariableIn != null && temVariableIn.length > 0) {
        ...
        for (int i = 0; i < temVariableIn.length; i++) {
          variablesIn[i] = temVariableIn[i].getAttributeValue("Value");
        }
      }
      ...
      attr = new String[] {
          "Name"};
      val = new String[] {
          FlowMetaConstants.PREVIEW_VARIABLE};
      Element[] temVariablePrev = ElementUtils.findElements(activityRoot, null, null,
          attr, val);
      if (temVariablePrev != null && temVariablePrev.length > 0) {
        ...
        for (int i = 0; i < temVariablePrev.length; i++) {
          variablesPreview[i] = temVariablePrev[i].getAttributeValue("Value");
        }
      }

      //活动转换限制:合并
      Element[] temJoinType = ElementUtils.findElements(activityRoot, "Join", null, null, null);
      ...
      //活动转换限制:分裂
      Element[] temSplitType = ElementUtils.findElements(activityRoot, "Split", null, null, null);
      ...
      //后继转换
      Element[] temPostTrans =
          ElementUtils.findElements(activityRoot, "TransitionRef", null, null, null);
      ...
      if(postTrans!=null&&postTrans.length>0){//有先后的区别
        workflowNode.setPostTrans(postTrans);
      }else{
        workflowNode.setPostTrans(getTransitionIDsFrom(flowProcessID, nodeID));
      }
      workflowNode.setPreTrans(this.getTransitionIDsTo(flowProcessID, nodeID));
...
//------------------------------------------------------------------------------
  public String[] getTransitionIDsFrom(String flowProcessID,
                                       String fromNodeID) {
    Element processRoot = this.getFlowProcessRoot(flowProcessID);
    String attr[] = new String[] {
        "From"};
...
//------------------------------------------------------------------------------
  public FlowProcTransition getTransitionByID(String flowProcessID,
    ...
    Element[] elemArray =
        ElementUtils.findElements(processRoot, "Transition", null, attr, val);
    if (elemArray == null || elemArray.length == 0) {
      throw new RuntimeException("找不到transition：" + transitionID);
    }
    else {
      ...
      Element conditions[] =
          ElementUtils.findElements(transRoot, "Condition", null, null, null);
...
//------------------------------------------------------------------------------
  public HashMap getDataFields(String flowProcessID) {
    Element processRoot = this.getFlowProcessRoot(flowProcessID);
    ...
    HashMap flowDataFields = new HashMap();
...
//------------------------------------------------------------------------------
  public String[] getAllNodeIDs(String flowProcessID) {
    Element processRoot = this.getFlowProcessRoot(flowProcessID);
    Element[] temNodes = ElementUtils.findElements(processRoot,
...
//------------------------------------------------------------------------------
  public FlowMetaFile[] getAllFlowMetaFiles() {
    Element[] temMetas = ElementUtils.findElements(root,
        "WorkflowProcess", null, null, null);
    if (temMetas == null) {
      return null;
    }
    ...
    for (int i = 0; i < temMetas.length; i++) {
      ...
      flowMetas[i].setAccessLevel(eleMeta.getAttributeValue("AccessLevel"));
