...
//------------------------------------------------------------------------------
  public ArrayList processNodes(Collection nodeColl) {
    ...
    while (it.hasNext()) {
      ...
      if (anode.getParentID() == null || anode.getParentID().trim().equals("")) { //has no father
        resultTree.add(new Integer(i).toString());
        resultTree.add(anode);
        if (this.hasSon(anode.getNodeID())) {
          makeTree(nodeColl, resultTree, anode, (new Integer(i)).toString());
        }
        i++;
      }
    }
...
//------------------------------------------------------------------------------
  private void makeTree(Collection source, ArrayList target,
...
//------------------------------------------------------------------------------
  private Collection printTree(ArrayList source) {
    ...
    while (it.hasNext()) {
      ...
      TreeNode node = (TreeNode) it.next();
      String img = "";
      if (this.hasSon(node.getNodeID())) { //-------------有儿子
        img = "<img src=\"" + contextPath + "/images/plus.gif\" border=0 id=" +
            id
            + " onClick=\"expandIt('" + id + "'); return false\">";
      }
      else {
        img = "<img src=\"" + contextPath + "/images/open.gif\" border=0 id=" +
            id + ">";
      }
      ...
      if (node.getParentID() == null || node.getParentID().trim().equals("")) { //----------------顶级分组
        if (this.checkboxName == null || this.checkboxName.trim().equals("")) {
          tableStr =
              "<table cellpadding=0 cellspacing=1 class=LayoutWebPartFrame " +
              " id=" + id + " style=\"display:block\">" +
              "<tbody><tr> <td valign=\"top\" class=LayoutWebPart>" + nbsp +
              "<img src=\"" + contextPath +
              "/images/spacer.gif\"  border=0 alt=\"\">" +
              img + "&nbsp;<a href="
              + url + " target=" + targetFrame + " onclick=changeColor('" + id +
              "','" + node.getNodeID() +
              "')><font size='' class=\"ldh-12-20\" color=#000000 id=" + id +
              ">" + node.getNodeName() + "</font></a></td>" +
              " </tr></tbody></table>";
          ...
          tableStr =
              ...
              "<img src=\"" + contextPath +
              "/images/spacer.gif\"  border=0 alt=\"\">" +
              img + "<input type=checkbox name=" + checkboxName + " value=" +
              node.getNodeID() + "><a href="
              + url + " target=" + targetFrame + " onclick=changeColor('" + id +
              "','" + node.getNodeID() +
              "')><font size='' class=\"ldh-12-20\" color=#000000 id=" + id +
              ">" + node.getNodeName() + "</font></a></td>" +
        ...
        if (this.checkboxName == null || this.checkboxName.trim().equals("")) {
          tableStr =
              "<table valign=\"top\" cellpadding=0 cellspacing=1 class=LayoutWebPartFrame " +
...
//------------------------------------------------------------------------------
  private Collection printTree(ArrayList source, Collection resourcesHasRight,
                               String webModuleID, String roleID) {
    ArrayList outPut = new ArrayList();
    Iterator it = source.iterator();
...
//            xinghao = "&nbsp;&nbsp;<a target='_top' title='删除之' href='"
//                + contextPath + "/rolemanage/remove_resource_role.do?roleID=" +
      ...
      if (node.getParentID() == null || node.getParentID().trim().equals("")) { //----------------顶级分组
        if (this.checkboxName == null || this.checkboxName.trim().equals("")) {
          tableStr =
              ...
              "<img src=\"" + contextPath +
              "/images/spacer.gif\"  border=0 alt=\"\">" +
              img + "&nbsp;<a href=" + url + " target=" + targetFrame +
              " onclick=changeColor('" + id + "','" + node.getNodeID()
              + "')><font size='' class=\"ldh-12-20\" color=#000000 id=" + id +
              ">" +
              node.getNodeName() + xinghao + "</font></a></td>" +
              " </tr></tbody></table>";
          ...
          tableStr =
              ...
              "<img src=\"" + contextPath +
              "/images/spacer.gif\"  border=0 alt=\"\">" +
              img + "<input type=checkbox name=groupToDel value=" +
              node.getNodeID() + "><a href="
              + url + " target=" + targetFrame + " onclick=changeColor('" + id +
              "','" + node.getNodeID() +
              "')><font size='' class=\"ldh-12-20\" color=#000000 id=" + id +
