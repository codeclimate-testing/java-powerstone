...
public class DriverOutputFormController
    extends SimpleFormController {
  private final Log log = LogFactory.getLog(DriverOutputFormController.class);
  ...
  public ModelAndView onSubmit(HttpServletRequest request,
    ...
    WFDriverOutputParam driverOutputParam = (WFDriverOutputParam) command;
    if (log.isDebugEnabled()) {
      log.debug("entering 'onSubmit' method..." + driverOutputParam);
    }
    workflowDriverManager.addDriverOutputParam(request.getParameter("driverID"),
                                               driverOutputParam);
    //打印刷新父窗口的代码到客户端
  ...
  protected Object formBackingObject(HttpServletRequest request) throws
    ...
    if (log.isDebugEnabled()) {
      log.debug("entering 'formBackingObject' method[driverID=" + driverID +
                "|driverOutputParamID=" + driverOutputParamID + "]");
    }
    return new WFDriverOutputParam();
  }
