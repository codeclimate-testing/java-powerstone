...
public class WorkflowEngineStarterTest extends AbstractSpringTestCase {
	...
	protected void onSetUpInTransaction() throws Exception {
		log = LogFactory.getLog(WorkflowEngineStarterTest.class);
		log.info("`````````````````````````");
		...
		try {
			...
			wm = flowMetaManager.uploadFlowMetaFile(new FileInputStream(
					...
					new FileInputStream(new File(fileName)), new Long(new File(
							fileName).length()), new Long(new File(fileName)
							.length()));
		...
		workflowDriverManager.addDriverOutputParam(wd1.getFlowDriverID()
				.toString(), wdOutParam1);
		workflowDriverManager.addDriverOutputParam(wd1.getFlowDriverID()
				.toString(), wdOutParam2);
		workflowDriverManager.addDriverOutputParam(wd1.getFlowDriverID()
				.toString(), wdOutParam3);
		workflowDriverManager.saveWorkflowDriver(wd1);

		log.info("部署流程" + wd2.getFlowDriverID());
		flowDeployManager.updateFlowNodeBinding(
		...
		flowDeployManager.updateFlowNodeBinding(
				fd.getFlowDeployID().toString(), "dreambike_Wor1_Act4", wd4
						.getFlowDriverID().toString());
		flowDeployManager.updateFlowNodeBinding(
				fd.getFlowDeployID().toString(), "dreambike_Wor1_Act6", wd5
						.getFlowDriverID().toString());
		FlowNodeBinding nodeBinding7 = flowDeployManager.updateFlowNodeBinding(
				fd.getFlowDeployID().toString(), "dreambike_Wor1_Act7", wd6
						.getFlowDriverID().toString());
		FlowNodeBinding nodeBinding8 = flowDeployManager.updateFlowNodeBinding(
				fd.getFlowDeployID().toString(), "dreambike_Wor1_Act8", wd7
						.getFlowDriverID().toString());
		FlowNodeBinding nodeBinding9 = flowDeployManager.updateFlowNodeBinding(
				fd.getFlowDeployID().toString(), "dreambike_Wor1_Act9", wd8
						.getFlowDriverID().toString());
		flowDeployManager.updateFlowNodeBinding(
				fd.getFlowDeployID().toString(), "dreambike_Wor1_Act10", wd9
						.getFlowDriverID().toString());
		flowDeployManager.updateFlowNodeBinding(
				fd.getFlowDeployID().toString(), "dreambike_Wor1_Act11", wd10
						.getFlowDriverID().toString());

		flowDeployManager.enableFounder(nodeBinding7.getNodeBindingID()
				.toString());
		...
		flowDeployManager.updateOtherPerformer(nodeBinding9.getNodeBindingID()
				.toString(), "dreambike_Wor1_Act4");

		HashMap inputParamMap = new HashMap();
		HashMap outputParamMap = new HashMap();
		outputParamMap.put("custome_ID", wdOutParam1.getDriverOutputParamID()
				.toString());
		outputParamMap.put("productID", wdOutParam2.getDriverOutputParamID()
				.toString());
		outputParamMap.put("custome_email", wdOutParam3
				.getDriverOutputParamID().toString());
		HashMap outputParamEnumeMap = new HashMap();

		flowDeployManager.updateFlowNodeParamBinding(fd
				.getFlowNodeBindingByNodeID("dreambike_Wor1_Act1")
				.getNodeBindingID().toString(), inputParamMap, outputParamMap,
				outputParamEnumeMap);

		log.info("指定参与者");
		flowDeployManager.addUserPerformer(user.getId().toString(), fd
				.getFlowNodeBindingByNodeID("dreambike_Wor1_Act1")
				.getNodeBindingID().toString());

		// flowDeployManager.enableFounder(fd.getFlowNodeBindingByNodeID(
		// "dreambike_Wor1_Act2").getNodeBindingID().toString());
		flowDeployManager.updateAssign(fd.getFlowNodeBindingByNodeID(
				"dreambike_Wor1_Act2").getNodeBindingID().toString(),
				"dreambike_Wor1_Act1");
		flowDeployManager.updateOtherPerformer(fd.getFlowNodeBindingByNodeID(
				"dreambike_Wor1_Act3").getNodeBindingID().toString(),
				"dreambike_Wor1_Act1");

		// flowDeployManager.enableFounder(fd.getFlowNodeBindingByNodeID(
		// "dreambike_Wor1_Act3").getNodeBindingID().toString());
		flowDeployManager.enableFounder(fd.getFlowNodeBindingByNodeID(
				"dreambike_Wor1_Act4").getNodeBindingID().toString());
		flowDeployManager.enableFounder(fd.getFlowNodeBindingByNodeID(
				"dreambike_Wor1_Act6").getNodeBindingID().toString());
		flowDeployManager.enableFounder(fd.getFlowNodeBindingByNodeID(
				"dreambike_Wor1_Act10").getNodeBindingID().toString());
		flowDeployManager.enableFounder(fd.getFlowNodeBindingByNodeID(
				"dreambike_Wor1_Act11").getNodeBindingID().toString());
	...
	public void testProcessActivityReport() {
		Integer myTasksToAssignNum = taskManager.findMyTasksToAssignNum(user
				.getId().toString());
		log.info("^^^^^^^^^^^^^^需要指派的任务数量[" + myTasksToAssignNum + "]");
		this.assertTrue("需要指派的任务数量==1", myTasksToAssignNum.intValue() == 1);
		while (true) {
			List allMyNewTasks = taskManager.findAllMyNewTasks(user.getId()
					.toString());
			...
			if (it.hasNext()) {
				while (it.hasNext()) {
					...
					taskManager.checkOutTask(user.getId().toString(), aTask
							.getTaskID().toString());
					workflowEngine.processSubmitTask(user.getId().toString(),
		...
		while (true) {
			List allMyExecutingTasks = taskManager
					.findMyExecutingTasksByType(user.getId().toString(), bm
							.getTypeID().toString(), 1, 1000);
		...
		log.info("Finished任务："
				+ taskManager.findMyFinishedTasksKinds(user.getId().toString())
						.size());
	}
