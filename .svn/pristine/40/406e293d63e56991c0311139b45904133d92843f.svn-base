package org.powerstone.acegi.service;
...
import org.powerstone.acegi.AuthenticationHelper;
...
public class DaoAuthenticationService extends JdbcDaoSupport implements AuthenticationService{
	...
	public List getResources() {
		...
		for (Iterator iter = resources.iterator(); iter.hasNext();) {
			...
			List auths = new AuthoritiesByResourcMapping(getDataSource()).execute(resc.getResString());
	...
	public List getUsers() {
		...
		for (Iterator iter = users.iterator(); iter.hasNext();) {
			...
			List auths = new AuthoritiesByUsernameMapping(getDataSource()).execute(user.getUsername());
			GrantedAuthority[] arrayAuths = AuthenticationHelper.convertToGrantedAuthority(auths);
