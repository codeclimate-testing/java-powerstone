...
import org.powerstone.workflow.service.*;
import org.powerstone.ca.model.User;
import org.powerstone.ca.model.Role;

public class WorkflowEngineImpl implements WorkflowEngine {
	private static Log log = LogFactory.getLog(WorkflowEngineImpl.class);

	public static final boolean VALIDATE_START_FLOW = false;

	// 发起进程时是否验证权限
	public boolean validateStartFlow = VALIDATE_START_FLOW;

	private FlowProcManager procManager;

	private FlowTaskManager taskManager;

	private FlowDeployManager deployManager;

	private FlowMetaManager flowMetaManager;

	private WorkflowDriverManager flowDriverManager;
	...
	private void generateFlowTask(FlowProc flowProc,
		...
		if (preTransaction != null) {
			procManager.mergeProcTransaction(procTransaction, preTransaction);
		...
		if (!nodeToProcess.isRouteNode()) { // 如果是普通节点：产生任务
			FlowTask flowTask = taskManager.createFlowTask(procTransaction,
			...
			if (nodeBinding.isStatic()) {
				for (Iterator it = deployManager.findUsersByNodeBinding(
					...
					taskManager.fenfaNewTask(auser.getId().toString(), flowTask
							.getTaskID().toString());
					if (log.isDebugEnabled()) {
						log.debug("任务[" + flowTask.getTaskID()
								+ "]流程参与者逻辑isStatic,分发给了用户[" + auser.getId()
								+ "]");
					}
				}
				for (Iterator it = deployManager.findRolesByNodeBinding(
					...
					for (Iterator it2 = role.getUsers().iterator(); it2
						...
						taskManager.fenfaNewTask(auser.getId().toString(),
				...
				if (!flowTask.hasTaskCandidate()) {
					taskManager.assignTask(flowTask.getFlowProc()
				...
				for (Iterator it = otherTask.getTaskUsers().iterator(); it
					...
					taskManager.assignTask(flowTaskUser.getUserID(), flowTask
							.getTaskID().toString());
				...
				String variablePerformer = (String) procState.get(nodeBinding
						.getPerformerDetail());
				taskManager.assignTask(variablePerformer, flowTask.getTaskID()
						.toString());
				...
				if (otherTask == null) {
					log.warn("流程[" + flowMetaWithFile.getFlowProcessID()
							+ "]的节点[" + nodeToProcess.getNodeName()
							+ "]没有对应的任务");
					taskManager.needAsssign(flowTask.getFlowProc()
							.getStarterUserID(), flowTask.getTaskID()
							.toString());
					if (log.isDebugEnabled()) {
				...
				for (Iterator it = otherTask.getTaskUsers().iterator(); it
						.hasNext();) {
					FlowTaskUser flowTaskUser = (FlowTaskUser) it.next();
					taskManager.needAsssign(flowTaskUser.getUserID(), flowTask
							.getTaskID().toString());
	...
	private void processTask(FlowTask flowTask) {
		...
		WorkflowMeta flowMetaWithFile = flowMetaManager
				.getWorkflowMetaWithFile(flowProc.getFlowDeploy()
		...
		if (!currNode.isRouteNode()) {
			procManager.completeTransaction(flowTask.getFlowProcTransaction());
			if (log.isDebugEnabled()) {
				log.debug("结束了一个事务["
	...
	private void routeFlow(FlowProc flowProc,
		...
		if (postTransRefs == null || postTransRefs.length == 0) { // 没有后继节点：进程结束或者只是一个分支结束
			// 结束事务，来防止最后一个任务取回
			procManager.completeTransaction(procTransaction);
			if (log.isDebugEnabled()) {
			...
			if (!procTransaction.isActive()) { // 如果前一个事务结束了：启动一个新的事务
				newTransaction = procManager.createProcTransaction(flowProc);
				if (log.isDebugEnabled()) {
			...
			if (splitType == null || splitType.equals("AND")) {
				for (int i = 0; i < postTransitionIDs.length; i++) {
					...
					// 记录工作流转换
					procManager.createFlowProcTransition(nodeID, ft
	...
	public void processSubmitTask(String userID, String taskID) {
		...
		FlowTask flowTask = taskManager.getFlowTask(taskID);
		// 判断任务是否处于正确的状态
		...
		if (linkedFlowProcs != null) {
			for (Iterator it = linkedFlowProcs.iterator(); it.hasNext();) {
				...
				// 进行工作流路邮
				routeFlow(flowProc,
						procManager.createProcTransaction(flowProc), // 新建事务
						flowMetaWithFile, currNode);
				if (log.isDebugEnabled()) {
					log.debug("流程自动连接:进程["
	...
	public void processActivityReport(String userID,
		...
		if (activityReport.getFlowTaskID() != null) {
			flowTask = taskManager.getFlowTask(activityReport.getFlowTaskID());
			flowDriver = flowTask.getFlowNodeBinding().getWorkflowDriver();
		} else if (driverID != null) {
			flowDriver = flowDriverManager.getWorkflowDriver(driverID);
		}

		// 查找相关的工作流进程,包括'自身'
		Collection relativeProcs = this.getRelativeProcs(userID, flowTask,
				flowDriver);
		...
		for (Iterator it = relativeProcs.iterator(); it.hasNext();) {
			...
			if (activityReport.getFlowTaskID() == null) {
				WorkflowMeta flowMetaWithFile = flowMetaManager
						.getWorkflowMetaWithFile(currFlowProc.getFlowDeploy()
								.getWorkflowMeta().getFlowMetaID().toString());
				...
				routeFlow(currFlowProc, procManager
						.createProcTransaction(currFlowProc), // 新建事务
						flowMetaWithFile, currNode);
			}
		}
	}
		...
		if (nodeBindings != null && nodeBindings.size() > 0) {
			for (Iterator it = nodeBindings.iterator(); it.hasNext();) {
				...
				WorkflowMeta flowMeta = flowMetaManager
						.getWorkflowMetaWithFile(nodeBinding.getFlowDeploy()
								.getWorkflowMeta().getFlowMetaID().toString());
	...
	private FlowProc startAFlow(FlowDeploy flowDeploy, FlowProc linkFlowProc,
			String startUser) {
		FlowProc flowProc = procManager.startAFlow(flowDeploy, linkFlowProc,
				startUser);
		if (log.isDebugEnabled()) {
	...
	public void setFlowDriverManager(WorkflowDriverManager flowDriverManager) {
